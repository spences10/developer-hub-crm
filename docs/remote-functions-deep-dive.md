...
> Based on Svelte Radio interview with Simon Holthausen (Svelte Core Team Lead)

This document provides a comprehensive understanding of SvelteKit remote functions, their design rationale, and best practices.
...
1. **Granularity Problem**
   ...
   - Reloading partial data requires workarounds (nested layouts, route groups)
   - Must re-fetch everything even if only one piece changed
   - Example: Dashboard with stats + contacts - can't refresh just stats
...
✅ **Granular data fetching** - Call queries anywhere in component tree
✅ **Better collocation** - Data fetching near where it's used
✅ **Full type safety** - Regular TypeScript functions, no magic
✅ **Flexible refresh** - Choose what to refresh, when
✅ **Single-flight mutations** - Refresh data in same request (performance!)
✅ **SPA mode support** - Commands for non-progressively-enhanced scenarios
...
**No need to hoist** - Use queries wherever you need them, automatic deduplication handles it.
...
**Purpose**: Mutations that require JavaScript (no progressive enhancement)
...
export const toggle_favorite = command(
  v.pipe(v.string(), v.minLength(1)),
  async (id) => {
    await db.query('UPDATE contacts SET favorite = NOT favorite WHERE id = ?', [id]);
    return { success: true };
  }
);
```
...
await toggle_favorite(id, {
  refresh: [get_contact],
  optimistic: (contact) => ({ ...contact, favorite: !contact.favorite })
});
```
...
export const get_comments = query(
  ...
  async (post_id) => {
    ...
    return await db.query('SELECT * FROM comments WHERE post_id = ?', [post_id]);
  }
);
```
...
**Problem**: Rendering a list where each item needs related data causes N+1 queries.
...
- If you're just returning a function that executes queries separately (defeats the purpose)
...
```svelte
<script>
  import { get_contacts, get_stats, get_follow_ups } from './data.remote';
</script>
...
**Benefit**: Runtime knows where nearest boundary is, coordinates all async work, shows single loading state.

---

## Authentication Patterns

**Problem**: Remote functions aren't tied to routes (unlike load functions).
...
export const guarded_form = <T>(schema: T, fn: (data: any) => any) => {
...
export const guarded_command = <T>(schema: any, fn: (data: T) => any) => {
...
```typescript
// contacts.remote.ts
import { guarded_query, guarded_command, get_current_user_id } from '$lib/server/auth-helpers';
...
- Can call `getRequestEvent()` in helper functions (not just remote functions)
...
```typescript
async function validate_ownership(user_id: string, contact_id: string) {
...
export const update_contact = guarded_command(schema, async (data) => {
  const user_id = await get_current_user_id();
...
- **Advanced caching**: ISR, provider-specific optimizations (Vercel, Cloudflare)
- **Resource API**: SPA-mode query capabilities without server
- **Automatic batching**: All queries batched by default (design question)
...
export const delete_contact = guarded_command(v.string(), async (id) => {
...
```typescript
export const create_interaction = guarded_form(schema, async (data) => {
...
2. ✅ **Use single-flight mutations** (call `.refresh()` in forms/commands)
3. ✅ **Always validate with schemas** - endpoints are public!
4. ✅ **Use auth helpers** for consistent authentication
5. ✅ **query.batch for actual batching** - or use regular `query()` if not batching
...
**Status**: Experimental feature (as of 2024)
